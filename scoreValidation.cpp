...
The input file contains several positive or negative floating-point (double) values​.
 ...
 After the input of 2 valid scores, an integer number X will be read. Your program must stop when X = 2.

Output :
If an invalid score is read, must be printed the message "nota invalida".

 When two valid scores are read, the message "media = " must be printed folowed by the average between these 2 scores.
